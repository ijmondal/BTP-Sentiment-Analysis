...
# Store the refined text in two different files(lang_tagged_text.txt: for finding CMI and text.txt
...
def extractFromAttributes():
	for dic in dataDict:
		t = str(dic['text']).encode('ascii','ignore').split()
		ltt = str(dic['lang_tagged_text']).encode('ascii','ignore').split()
		t_final = []
		ltt_final = []

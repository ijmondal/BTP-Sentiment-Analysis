...
if __name__ == "__main__":
    ...
    data = read_data("3_text.txt")
    print("Cleaning words")
    broken_words = break_in_subword(data)
    print("Writing words")
    fp=open("write.txt","w")
    # print(broken_words[1][0][0])
    # print(len(broken_words))
    for i in range(len(broken_words)):
        for j in range(len(broken_words[i])):
            for k in range(len(broken_words[i][j])):
                fp.write(str(broken_words[i][j][k]))
                fp.write(" ")
        fp.write("\n")
